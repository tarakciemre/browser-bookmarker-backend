...
async function logIn(username, password) {
  try {
    ...
    const userResults = await db.execute(query, [username]);
    if (userResults.rows.length === 0 || !checkLogin(password, userResults.rows[0].password)) {
      throw new Error('Invalid username or password');
    }

    // create a login token
    const token = uuidv4();
    const tokenQuery = "INSERT INTO loggedInUsers (token, username) VALUES (?, ?)";
